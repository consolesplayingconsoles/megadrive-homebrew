...
Card_Loop:
	...
	.frameIdSet:
		...
		bne.s	.setupCardObject			; if not, set it up normally
		; sonic-vs-robotnik: these are boss duels, not "act 3 of a run" -- drop the
		; ACT number. Keep the object so the title-card wait loop (needs all 4
		; elements in place) still completes, but point it at an empty mapping.
		clr.b	obFrame(a1)				; frame 0 of the empty mapping
		move.l	#Map_CardBlank,obMap(a1)		; empty mapping -> invisible
		bra.s	.cardObjectCommon
	; Card_MakeSprite:
	.setupCardObject:

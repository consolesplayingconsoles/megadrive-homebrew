...
; loc_1895C:
BossStarLight_Done:
		; sonic-vs-robotnik: this build is FixBugs=0, whose seesaw scan starts at the
		; wrong slot and only covers HALF of object RAM ("nonsensical" per the disasm)
		; -- so a seesaw in the 2nd half (the left one) is never found and can't be
		; shot. Force the correct full scan so all 3 seesaws are always in the list.
		lea	(v_lvlobjspace).w,a1 			; correct start of dynamic object space
		lea	BossStarLight_SeesawList(a0),a2 	; scratch RAM to collect seesaw pointers
		moveq	#id_Seesaw,d0
		moveq	#(v_lvlobjend-v_lvlobjspace)/object_size-1,d1	; scan the WHOLE object space
...
; loc_18A5E:
BSLZ_ShipMove:
		; sonic-vs-robotnik: P2 drives Eggman left/right (velocity-clamped). The game's
		; own seesaw-drop (.dropSetup below) still fires when he lines up over a seesaw
		; -- the ball is welded to the seesaw so it can't be a free projectile. No
		; BossMove here (BSLZ_ShipUpdate does it); .dropSetup re-reads obX, so the pad
		; read in d0 is only needed for the movement above.
		...
		andi.b	#$3F,d1
		or.b	d1,d0
		not.b	d0					; d0 = Start A C B R L D U (pressed)
		move.w	#0,obVelX(a0)				; idle unless P2 pushes
		btst	#bitL,d0				; left?
		beq.s	.notleft
		cmpi.w	#boss_slz_x+8,obBossX(a0)		; already at the left wall?
		ble.s	.notleft
	...
	.notleft:
		btst	#bitR,d0				; right?
		beq.s	.notright
		cmpi.w	#boss_slz_x+$138,obBossX(a0)		; already at the right wall?
		bge.s	.notright
		move.w	#$200,obVelX(a0)
		bset	#0,obStatus(a0)				; face right
	.notright:
		; sonic-vs-robotnik: launch ON DEMAND -- only try to drop a ball when P2 holds
		; a face button (A/B/C). Without this Eggman dropped balls by himself. d0 still
		; holds the pad read here (.dropSetup overwrites it on its first line).
		btst	#bitB,d0
		bne.s	.dropSetup
		btst	#bitC,d0
		bne.s	.dropSetup
		btst	#bitA,d0
		beq.w	BSLZ_ShipUpdate				; no button -> no drop this frame
...
.findSeesaw:
		...
		; sonic-vs-robotnik: do NOT skip a seesaw Sonic is standing on -- P2 should be able
		; to bomb it on purpose (it tags him or flings him off; no glitch). Was btst #3.
		move.w	obX(a3),d3				; get the x position of the seesaw
		sub.w	d0,d3					; sonic-vs-robotnik: distance from the seesaw CENTRE
		move.w	d3,d5					; (do NOT add the $28 side offset here -- that put
		bpl.s	.posdist				; each drop zone $28 to the seesaw's right, killing
		neg.w	d5					; the left of every seesaw + the whole left arena)
	.posdist:
		cmpi.w	#$34,d5					; anywhere within +/-$34 of a seesaw drops on it
		blo.s	.prepDrop				; ($34 ~= half the seesaw spacing, so no dead gaps)
...
.prepDrop:
		...
		move.b	#15,BossStarLight_GenericTimer(a0)	; sonic-vs-robotnik: short post-drop lock (was 40) so P2 can pressure
		bra.w	BSLZ_ShipUpdate
; ===========================================================================

; BossStarLight_MakeBall:
BSLZ_MakeBall:
		cmpi.b	#15,BossStarLight_GenericTimer(a0)	; have we waited the (shortened) lock?
		...
		lea	(v_lvlobjspace).w,a1			; sonic-vs-robotnik: full scan (FixBugs=0 used a
		moveq	#(v_lvlobjend-v_lvlobjspace)/object_size-1,d1 ; half-range that misses 2nd-half balls)

; loc_18AFA:
; sonic-vs-robotnik: block a new drop ONLY if a real spike ball is still FALLING (routine 2)
; toward THIS seesaw. Excludes the explosion object (id_Explosion) and shrapnel (routine $A)
; that used to false-match by scratch word and wedge the left seesaw; also frees the seesaw
; the instant a ball lands (routine 4+), so P2 can cascade drops.
.checkForBall:
		cmpi.b	#id_BossSpikeball,obID(a1)		; a spike ball?
		bne.s	.nextBall
		cmpi.b	#2,obRoutine(a1)			; still falling (not landed/exploded/shrapnel)?
		bne.s	.nextBall
		cmp.l	BossSpikeball_SeesawPtr(a1),d0		; tied to THIS seesaw?
		beq.s	.abortDrop
	.nextBall:
		adda.w	#object_size,a1				; next object slot
		dbf	d1,.checkForBall
...
BSLZ_VsWin:
		; sonic-vs-robotnik: win -> level picker. This was gated behind `if FixBugs`, but the build
		; is FixBugs=0, so the win fell through to a plain delete and the level completed to the NEXT
		; act instead of the picker. Set id_Title UNCONDITIONALLY (matches the GHZ/MZ/SYZ win hooks).
		move.b	#id_Title,(v_gamemode).w
	if FixBugs
		addq.l	#4,sp					; avoid the display-and-delete bug
	endif
...
BossSpikeball_Main:	; Routine 0
		...
		; sonic-vs-robotnik: no seesaws on the Swiss Army Knife (Spring Yard) arena, so the
		; bomb is standalone -- base it on its own spawn position and just fall.
		cmpi.w	#id_SYZ_act3,(v_zone_act).w
		bne.s	.seesawpath
		move.w	obX(a0),obBossX(a0)			; standalone: base X on our own spawn X
		bra.s	.skip
	.seesawpath:
...
BossSpikeball_Fall:	; Routine 2
		jsr	(ObjectFall).l
		; sonic-vs-robotnik: standalone bomb (Spring Yard arena) -- fall until it hits the
		; floor, then explode; if it drops into a pit, just delete. No seesaw landing calc.
		cmpi.w	#id_SYZ_act3,(v_zone_act).w
		bne.s	.seesawland
		; the SYZ boss arena floor is block OBJECTS (no level collision), so land at the
		; fixed block-floor level (where Sonic stands) rather than via ObjFloorDist.
		cmpi.w	#boss_syz_y+$A0,obY(a0)			; reached the block floor (slightly lower)?
		bcs.s	.exitstd				; not yet -> keep falling
		move.w	obY(a0),BossSpikeball_SeesawY(a0)	; hit floor: MakeFrag reads SeesawY for the explosion Y
		move.w	#32,obSubtype(a0)			; seed the explosion (Explode checks #32)
		move.b	#8,obRoutine(a0)			; -> BossSpikeball_Explode (point detonation on the floor)
	.exitstd:
		rts
	.seesawland:
		movea.l	BossSpikeball_SeesawPtr(a0),a1		; copy offset address, a1 now contains the seesaw that this ball is tied to
...
; loc_18DAE:
.landed:
		move.w	#60,obSubtype(a0)			; sonic-vs-robotnik: brief sit then explode (was 240) -- the bomb no longer overstays

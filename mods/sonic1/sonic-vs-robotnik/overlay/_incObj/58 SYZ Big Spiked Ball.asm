...
BBall_Main:	; Routine 0
		...
		moveq	#$50,d0					; vanilla level spikeballs: full 80px orbit
		tst.l	objoff_34(a0)				; sonic-vs-robotnik: is this Eggman's wrecking ball?
		beq.s	.setRadius
		moveq	#$44,d0					; yes -> 68px orbit: low point reaches the floor directly under Eggman (hits standing AND ducking there), rising away to the sides so it only clips standing Sonic, then misses -- hide by moving off his column. Inner edge still clears his 48x48 body by 28px, no swipe
	.setRadius:
		move.b	d0,bball_radius(a0)			; set radius of circle motion (subtype $x3 only)
; ---------------------------------------------------------------------------

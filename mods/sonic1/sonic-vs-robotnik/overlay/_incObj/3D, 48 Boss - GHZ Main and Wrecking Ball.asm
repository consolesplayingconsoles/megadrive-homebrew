...
BGHZ_ShipMove:
		bsr.w	Robotnik_ReadP2				; d0 = port 2 d-pad (1 = pressed)
		move.w	#0,obVelX(a0)				; idle unless P2 pushes
		btst	#bitL,d0
		beq.s	.p2_chkright
		move.w	#-$140,obVelX(a0)			; P2 holds left
		bset	#0,obStatus(a0)
	.p2_chkright:
		btst	#bitR,d0
		beq.s	.p2_apply
		move.w	#$140,obVelX(a0)			; P2 holds right
		bclr	#0,obStatus(a0)
	.p2_apply:
		bsr.w	BossMove
		move.w	(v_screenposx).w,d1			; locked camera left edge
		move.w	obBossX(a0),d0
		move.w	d1,d2
		addi.w	#$20,d2					; left wall
		cmp.w	d2,d0
		bge.s	.p2_chkrwall
		move.w	d2,d0
	.p2_chkrwall:
		addi.w	#$120,d1				; right wall
		cmp.w	d1,d0
		ble.s	.p2_writex
		move.w	d1,d0
	.p2_writex:
...
Robotnik_ReadP2:	; poll player 2's pad directly (Sonic clobbers v_jpadhold2)
		move.b	#$40,(port_2_data).l			; TH high -> d-pad + B/C on the data lines
		nop
		nop
		move.b	(port_2_data).l,d0			; up/dn/lf/rt/B/C in bits 0-5
		not.b	d0					; 1 = pressed
		rts
; ===========================================================================
...
BGHZ_ShipDel:
		move.b	#id_Title,(v_gamemode).w		; sonic-vs-robotnik: Eggman fled -> Sonic WINS -> level picker
	if FixBugs
...
.skip:
		...
		cmpi.b	#4,ob2ndRout(a1)			; sonic-vs-robotnik: ship is in the P2 ShipMove routine
		bne.s	.checkAnchor				; if not, branch

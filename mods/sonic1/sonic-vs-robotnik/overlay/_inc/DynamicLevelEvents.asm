...
DLE_SYZ3_Boss:
	...
	.fail:
		move.w	#bgm_Boss,d0
		; sonic-vs-robotnik: the Swiss Army Knife bonus (SYZ arena, picker row 4) keeps the Special
		; Stage music instead of the boss theme -- this boss-trigger is what was overriding the
		; level-start bgm_SS with bgm_Boss.
		cmpi.w	#4,(v_levselitem).w
		bne.s	.vs_bossbgm
		move.w	#bgm_SS,d0
	.vs_bossbgm:
		bsr.w	QueueSound1				; play boss music (or Special Stage music for the bonus)
		move.b	#1,(f_lockscreen).w			; lock screen
		moveq	#plcid_Boss,d0
		bra.w	AddPLC					; load boss gfx
; ===========================================================================

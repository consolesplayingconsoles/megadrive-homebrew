...
; ---------------------------------------------------------------------------
; Infinite jump: a fresh jump press in mid-air relaunches Sonic upward, any
; number of times. Vanilla Sonic otherwise.
; ---------------------------------------------------------------------------
Sonic_AirJump:
		move.b	(v_jpadpress2).w,d0			; freshly pressed buttons this frame
		andi.b	#btnABC,d0				; jump tapped again?
		beq.s	.ret
		move.w	#-son_jumpspeed,obVelY(a0)		; relaunch upward
		bset	#2,obStatus(a0)				; curl into a ball (attack state)
		move.b	#id_Roll,obAnim(a0)			; jump/roll animation
		move.w	#sfx_Jump,d0
		jmp	(QueueSound2).l				; (returns)
.ret:
		rts
; ===========================================================================
...
Sonic_MdJump:	; While Sonic is in the air but not rolling
		...
		bsr.w	Sonic_AirJump				; infinite jump: tap jump in the air to relaunch
		bsr.w	Sonic_LevelBound			; make sure Sonic stays within level bounds and handle bottomless pits

...
Got_Loop:
		...
		; Infinite-GHZ: no "ACT N" on the results card either -> render nothing
		clr.b	obFrame(a1)				; frame 0 of the empty mapping
		move.l	#Map_CardBlank,obMap(a1)		; empty mapping -> invisible
		bra.s	.cardObjCommon
	; loc_C5CA:
	.setFrame:
...
Got_NextLevel:	; Routine $A
		; Infinite-GHZ "Groundhog Day": there is no Act 2. Finishing the act
		; always reloads Green Hill Act 1 with a freshly-rolled variant, forever.
		clr.w	(v_zone_act).w				; next level is always GHZ1
		clr.b	(v_ghz_variant).w			; re-roll the generated variant on reload
...
; ===========================================================================
	...
	; loc_C6EA:
	.restartLevel:
		move.w	#1,(f_restart).w			; trigger level restart to reload GHZ1

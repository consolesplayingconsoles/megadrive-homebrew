...
Card_Loop:
	...
	.frameIdSet:
		...
		; Infinite-GHZ Groundhog Day: no "ACT N". Keep the object so the title-card
		; wait loop (which needs all 4 elements to reach position) still completes,
		; but point it at an empty mapping so it draws nothing.

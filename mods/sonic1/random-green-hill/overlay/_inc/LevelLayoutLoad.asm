...
LevelLayoutLoad2:
		...
		; --- Infinite-GHZ: when loading Green Hill Act 1's foreground layout,
		; --- swap in one of the generated variants. The pick is made once per
		; --- session from the VBlank counter (random, based on how long the
		; --- title screen was up) and latched in v_ghz_variant, so dying and
		; --- retrying gives the SAME level; only a reset re-rolls it.
		lea	(Level_GHZ1).l,a0			; only the GHZ1 FG load dereferences this exact pointer (a0 is scratch here)
		cmpa.l	a0,a1
		bne.s	.noVariant
		moveq	#0,d0
		move.b	(v_ghz_variant).w,d0			; already picked this session?
		bne.s	.havePick				; yes -> reuse latched index
		move.b	(v_vblank_byte).w,d0			; no -> entropy: frames since power-on (low byte)...
		add.b	(v_random+3).w,d0			; ...mixed with the RNG state (chaotic, differs every run)
		andi.w	#31,d0					; 0..31 (32 variants)
		addq.w	#1,d0					; store as 1..32 (0 stays "unpicked")
		move.b	d0,(v_ghz_variant).w			; latch for the session (survives death, cleared on reset)
	.havePick:
		subq.w	#1,d0					; back to 0..31 index
		add.w	d0,d0					; *4 for long pointers
		add.w	d0,d0
		lea	(GHZ1_Variants).l,a0
		movea.l	(a0,d0.w),a1				; a1 = chosen variant layout
	.noVariant:
